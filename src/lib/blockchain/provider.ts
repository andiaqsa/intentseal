...
export function getReadProvider(): JsonRpcProvider {
  readProvider ??= new JsonRpcProvider(BOT_CHAIN.rpcUrl, {
...
export async function getInjectedChainId(): Promise<string | null> {
...
export async function switchToBotChain(): Promise<void> {
  if (!window.ethereum?.isMetaMask) throw new Error("METAMASK_UNAVAILABLE");
...
function getProviderErrorCode(error: unknown): number | undefined {
  ...
  if ("data" in error) return getProviderErrorCode(error.data);
  return undefined;
}
