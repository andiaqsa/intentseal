...
export function toUserError(
  ...
  if (
    ...
    if (context === "network") {
      return "The network switch was cancelled in MetaMask.";
    }

    if (context === "wallet") {
      return "The wallet connection was cancelled.";
    }

    return "The transaction was cancelled in MetaMask.";
  }
  ...
  if (
    message.includes("notintentcreator")
  ) {
  ...
  if (
    message.includes("intentnotfound") ||
  ...
  if (
    ...
    return `${BOT_CHAIN.chainName} could not be reached. Check your connection and try again.`;
  }

  if (context === "read") {
    return `The intent could not be read from ${BOT_CHAIN.chainName}.`;
  }

  if (context === "network") {
    return `MetaMask could not switch to ${BOT_CHAIN.chainName}.`;
  }

  if (context === "wallet") {
    return "MetaMask could not connect this wallet.";
  }
